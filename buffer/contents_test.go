...
func TestLines(t *testing.T) {
	...
	// contents argument
	if *buffer.EmptyLine(nil, nil) != *buffer.NewLine("", nil, nil) {
		t.Fatalf("Lines created by \"EmptyLine\" and \"NewLine\" are not the same")
	}

	// Test adding lines
	currentLine := buffer.EmptyLine(nil, nil)
	firstLine := currentLine
	for i := 0; i < 998; i++ {
		currentLine = buffer.EmptyLine(nil, currentLine)
	}
...
func benchmarkAddLines(num int, str string, b *testing.B) {
	for n := 0; n < b.N; n++ {
		testLine := buffer.NewLine(str, nil, nil)
		for i := 0; i < (num - 1); i++ {
...
func benchmarkCopyLines(numLines int, contained string, b *testing.B) {
	testLine := buffer.NewLine(contained, nil, nil)
